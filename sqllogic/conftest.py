...
def pytest_addoption(parser: pytest.Parser):
    ...
    parser.addoption(
        ...
        action="store",
        type=pathlib.Path,
        help="Path to the DuckDB root directory",
    )
    parser.addoption(
        "--build-dir",
        type=str,
...
def pytest_generate_tests(metafunc: pytest.Metafunc):
    ...
    root_dir: typing.Optional[pathlib.Path] = metafunc.config.getoption("duckdb_root_dir")

    if not root_dir:
        warnings.warn("'duckdb-root-dir' was not given, can't execute the tester")
        return

    parameters = []

    if test_glob:
        test_paths = root_dir.rglob(test_glob)
        parameters.extend(create_parameters_from_paths(test_paths, root_dir, metafunc.config))
    ...
    if parameters == []:
        if len(test_dirs) == 0:
            ...
            parameters.extend(scan_for_test_scripts(root_dir / "test", metafunc.config))
